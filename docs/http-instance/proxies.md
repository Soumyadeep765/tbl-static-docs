...
Route outbound `HTTP` requests through a proxy server. Useful when an API expects a specific network path, when you run your own proxy infrastructure, or when you need SOCKS tunneling.
...
The `proxy` option accepts a full URL with protocol, host, port, and optional credentials.
...
Any other protocol throws a validation error before the request is sent.
...
Store proxy credentials in [ENV variables](../globals/process.md), not in script source:
...
For most bot integrations with REST APIs, `http://` or `https://` is enough.
...
Use `proxy` when you already have a proxy service. Use `cfProxy` when you want a free, self-hosted Cloudflare edge router — see [Cloudflare Worker Proxy](cf-proxy.md).
...
| Works locally, fails in bot | Proxy not reachable from TBL servers | Use a publicly reachable proxy or [cfProxy](cf-proxy.md) |
